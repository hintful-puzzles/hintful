...
`n < 4` and an unreasonably large `n`. The game SHALL provide `solve`, and SHALL NOT provide `statusbarText` (faithfully to upstream) or `textFormat` (the upstream text format exists only in the excluded
