...
marks of its row and column), and a keep-mouse-highlight-after-pencil preference
(default off, matching upstream `PREF_PENCIL_KEEP_HIGHLIGHT`). Preference values
SHALL live on the `Ui` and be set as defaults by `newUi`. With auto-pencil off (the
default), note cleanup is manual — the player removes obvious candidates via the
mark-all control or a hint.
