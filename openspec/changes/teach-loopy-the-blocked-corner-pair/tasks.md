...
- [ ] 3.1 One sentence naming the contradiction rather than the shape. Draft in the
      proposal; the owner asked for a phrasing and gets the final say.
- [ ] 3.2 Say "lines reach both ends", not "incoming edges": the player sees lines on a
      board, not a direction of travel, and `help/games/loopy.md` now defines *edge* and
      *line* precisely enough that the sentence must respect the distinction.
- [ ] 3.3 The marks: the two dots that are full, the edge being ruled out, and the three
      being drawn. One firing, so one journey.
