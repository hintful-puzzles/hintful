...
**Reason**: drag entry is retired. Guess's was the collection's only
*token-transport* drag — picking a value out of a legend and carrying it to a
slot — and since `give-guess-element-keys` a panel key has carried the same one
bit in a single tap. Twenty-seven of the collection's games drag, and every
other one of them carries information no tap could reproduce: a path, a line
fill, a rubber band, an aim, or a piece whose position *is* the state. A drag
whose entire payload a button can deliver is pure cost, and it came with a
mechanical wart visible on a mouse as well as a finger — a tap on the palette
column armed a drag, placed nothing, and hid the cursor, so the intuitive "tap a
color, tap a slot" failed *and* took away the thing the panel acts on.

**Migration**: nothing a player's saved game notices — every drag field was
transient and a Guess save carries only the move log. Dragging a color onto a
slot becomes a press of that color's key; dragging a peg out of the row to clear
it becomes Clear, which now backspaces; dragging a color out of a past guess
becomes reading that color and pressing its swatch. The board's palette column,
whose only job was to be dragged from, goes with the drag. Replaced by "Guess
composes a row from colors, holds, keyboard, and hint input" below, which
carries this requirement's three scenarios unchanged.

### Requirement: Guess never writes past the end of the working row

**Reason**: the invariant survives; the behavior that enforced it does not.
Declining the erase key on the submit position left Clear dead at the moment a
typo is most likely and most worth correcting — the cursor rests there the
instant a row is full. Replaced by "Guess rubs out a color without ever
lengthening the row" below, which keeps the invariant and makes the key work.

**Migration**: none. The key now backspaces where it used to decline; no saved
data and no game ID is involved.
