...
**Reason**: Matching upstream's C output was a porting-era aid, retired by the owner on 2026-09-13 as no longer a concern. It required an advisory check generating boards from the C build and the TS port side by side, and a gated check that the TS solver grade every C-built reference board at exactly its C-recorded difficulty.

**Migration**: Correctness is "Galaxies generates uniquely-solvable boards at the requested difficulty". `galaxies-differential.test.ts` and its frozen `galaxies-c-reference.json` stay as a refactoring net; a deliberate divergence retires them.
