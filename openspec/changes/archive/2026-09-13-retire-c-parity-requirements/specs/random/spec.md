...
**Reason**: It required byte-identity with upstream's `random.c`, and its scenarios compared against "the C impl". Matching upstream's C was a porting tool; with the port finished it is not a requirement (owner, 2026-09-13), and a heading or scenario that names it cannot be renamed by a `MODIFIED` block.

**Migration**: Replaced by "The random module's output is stable across builds", which keeps the corpus replay and every edge case, stated as agreement with the committed corpus — the property shared game IDs actually depend on.
