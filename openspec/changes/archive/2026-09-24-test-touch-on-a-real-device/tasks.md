...
Done 2026-09-24: the laptop half against <https://hintful.click>, then a
big-picture device pass by the owner in the installed app. A tick means the item
has a verdict in `device-report.md`, and **"not tried" is one of them**. The
owner chose a ten-item subset and left the rest (Cube, Fifteen, Sokoban,
Pattern, the Mines two-finger tap, Bridges, Palisade, the keyboard cursor,
tablets) to continued testing.
...
- [x] 1.1 **Pegs** — press a peg, pause to choose a landing square, drag, lift.
      The hardest case and the one the fix was verified against in Chrome; if
      the repair shows anywhere, it shows here.
- [x] 1.2 **Filling** — press, pause, then drag across a run of cells.
- [x] 1.3 Cube · 1.4 Fifteen · 1.5 Flip · 1.6 Flood · 1.7 Sokoban — a held tap
      must act. Each was previously inert when held.
- [x] 1.8 For any that still feels wrong, say **which** of the two it is: the
...
- [x] 2.1 **Mines** — right-button flagging by long press and by two-finger tap
      must still work. It keeps the promotion deliberately.
- [x] 2.2 **Pattern** and **Loopy** — the two games with bespoke touch handling
      (`wantsStylusModifier`), where a tap cycles through three states because
      there is no second button. Most likely to surprise.
- [x] 2.3 **Tracks** — uses the secondary button without declaring it needs one.
...
- [x] 3.1 **Hit targets.** Can a fingertip reliably hit a Loopy edge, an
...
- [x] 3.2 **The on-screen keypad on a small screen.** Twelve games have one, and
...
- [x] 3.3 **The keyboard cursor**, if a phone keyboard is attached or on a
      tablet — the audit proved 56 games can be played by keyboard alone, and
      nobody has seen the cursor on a small screen.
- [x] 3.4 **Screen sizes and orientation.** Portrait and landscape, one small
...
- [x] 4.1 **Install** to the home screen, and launch from that icon. Check the
...
- [x] 4.3 **Update flow.** There is no prompt: an installed app auto-updates
...
- [x] 4.4 **Backgrounding.** Switch apps mid-game and come back; check a timed
...
- [x] 4.6 **Then** decide what is missing. Candidates, none to be built before
...
- [x] 5.1 `secondaryButtonHoldTime` (350 ms) and `secondaryButtonDragThreshold`
...
- [x] 5.2 If either wants changing, `src/utils/touch.test.ts` pins the behavior
...
- [x] 6.1 Fix inline what is local and unambiguous.
- [x] 6.2 File anything needing an interaction designed, with the finding
      quoted rather than a TODO.
- [x] 6.3 `ts-migration` spec: touch acceptance is a device activity, and the
      in-process tiers plus a synthetic-pointer browser pass are not it.
- [x] 6.4 `help/features.md` if any touch affordance turns out to need
      explaining to a player.
- [x] 6.5 `openspec validate test-touch-on-a-real-device --strict`.
- [x] 6.6 **Owner acceptance of the seven repaired games** — the item the input
