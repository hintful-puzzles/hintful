...
* `src/assets/` holds only committed files — nothing under `src/` is
  generated. `src/assets/icons/` is a committed thumbnail snapshot (see
...
[help/](help), in one format: site-level pages at the top level and one page
per game in [help/games/](help/games), all of them this project's own markdown.
`src/help-coverage.test.ts` holds that directory and the catalog to each other
in both directions.
...
TypeScript, so there is no compilation step ahead of it, and nothing is
generated.
...
That is the entire setup, on any platform. There was a `Brewfile` until
recently: it carried Emscripten, cmake and jq for the WASM puzzle engine, which
was retired once every game had a native TypeScript implementation
(`retire-c-engine`, 2026-08-01), and then halibut alone for the in-app manual,
which was deleted with the manual (`retire-the-upstream-help-tree`). The
repository now has no native toolchain dependency of any kind.
...
A fresh clone runs `npm run dev` and `npm run build` with nothing built first
and nothing installed but Node: the game catalog is committed source
(`src/puzzle/catalog-data.ts`), the icons are a committed snapshot, and the help
pages are committed markdown.
