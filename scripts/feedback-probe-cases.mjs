    ...
    cases: [
      {
        ...
        find: "    if (fired.tier > grade) grade = fired.tier;",
        replace: "    grade = fired.tier;",
      },
      {
        within: "runDeductionFixpoint",
        why: "the grade is the technique's position in the ladder, not its declared tier",
        find: "    if (fired.tier > grade) grade = fired.tier;",
        replace: "    grade = opts.techniques.indexOf(fired);",
      },
      {
        within: "firstFiring",
        why: "the ladder does not restart from the top after a rung fires",
        // The whole pass, because the plant needs a variable of its own: every
        // rung is tried after a firing, and the pass reports the last to fire.
        find: [
        ...
        replace: [
          "  let last: DeductionTechnique | null = null;",
          "  for (const technique of techniques) {",
          "    if (maxTier !== undefined && technique.tier > maxTier) continue;",
          "    beforeTechnique?.(technique);",
          "    const ret = technique.run();",
      ...
      },
      {
        ...
        find: '    if (ret < 0) return "contradiction";',
        replace: "    if (ret < 0) continue;",
      },
      {
        within: "singleFirings",
        why: "a contradiction is forgotten, so the next call runs the ladder again",
        find: "      if (impossible) return null;",
        replace: "      void impossible;",
      },
      {
        within: "firstFiring",
...

  {
    ...
    cases: [
      ...
      },
      {
        ...
        find: '      (op) => (op.reason as { kind?: string }).kind !== "dup",',
        replace: "      (op) => true,",
      },
      {
        within: "availableStrikes",
        why: "a fully-struck firing ends the search instead of advancing to the next live one",
        find: "    if (teachable.length > 0) {",
        replace: "    if (teachable.length === 0) break;\n    {",
      },
      {
        within: "availableStrikes",
        why: "a later firing is offered while an earlier one has yet to strike its premise",
        find: "!premise.some((c) => pending.has(c))",
        replace: "true",
      },
      {
        within: "nextPlace",
