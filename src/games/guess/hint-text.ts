 ...
 * and nothing else: the **striped row** is the scored guess a step reads (the
 * hint's line hatch, docs/games/hints.md § "Hatch the line the sentence
 * names"), an **outlined answer slot** is one whose marks it leans on, and the **framed
...
export function say(r: Reason): string {
  switch (r.kind) {
    case "scoredNothing":
      return "The striped row scored nothing, so none of its colors can be in the answer. Rule them out everywhere.";
    case "noBlack":
      return "The striped row scored no black pegs, so none of its colors can be where it was guessed. Rule each out there.";
    case "everyPegScored":
      return "Every peg of the striped row scored, so the answer can only use its colors. Rule the rest out.";
    case "noRepeats":
      return "The outlined slot can only be one color, and no color repeats, so no other slot can hold it.";
    case "blacksForced":
      return r.black === 1
        ? "Only 1 peg of the striped row can still be in place, and it scored 1 black, so that peg must be right."
        : `Only ${r.black} pegs of the striped row can still be in place, and it scored ${r.black} black, so all must be right.`;
    case "blacksAccounted":
      return r.black === 1
        ? "The outlined slot already accounts for the striped row's black peg, so none of its other pegs can be in place."
        : `The outlined slots already account for the striped row's ${r.black} blacks, so none of its other pegs can be in place.`;
    case "totalAccounted":
      return `The outlined slots account for all ${pegs(r.total)} the striped row scored, so its colors can't be anywhere else.`;
    case "onlyAnswer":
