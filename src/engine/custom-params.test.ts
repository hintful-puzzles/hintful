...
// populated the registry. A sweep driven by a list can only be as good as the
// list, so the count is asserted below rather than assumed: an empty registry
// would generate no `it()` at all and the file would pass in silence.
